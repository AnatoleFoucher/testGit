...
void TIM5_IRQHandler(void){
	...
	HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_5); //changement d'état du buzzer
}
...
void TIM2_IRQHandler(void){
	...
	switch(nombre){
		case 7:
		timer2_rythme.Init.Period = 500;
		HAL_TIM_Base_Init(&timer2_rythme);
		break;

		case 14:
		timer2_rythme.Init.Period = 250;
		HAL_TIM_Base_Init(&timer2_rythme);
		break;

		case 40:
		timer2_rythme.Init.Period = 100;
		HAL_TIM_Base_Init(&timer2_rythme);
		break;

		case 60:
		timer2_rythme.Init.Period = 60;
		HAL_TIM_Base_Init(&timer2_rythme);
		break;
...
static void initialisation_timer2_rythme(void){
	...
	timer2_rythme.Init.Prescaler = 16000;
	timer2_rythme.Init.Period = 1000; //nombre à atteindre pour avoir un switch par seconde, no prescaler
